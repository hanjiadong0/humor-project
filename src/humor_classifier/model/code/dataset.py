...
class Dataset ():
    ...
    def calculate_language (self, label = "tweet", default_language = 'es', threshold = 0.75):
        ...
        def func (tweet):
            ...
            prediction = model(tweet)[0]
            
            # @var first_language String
            first_language = prediction['label']

            # @var first_language_affinity float
            first_language_affinity = prediction['score']

            # Return the language if exceeds the threshold
            return first_language if first_language_affinity > threshold else None

        # Assign the language
        self.df['language'] = self.df[label].apply(func)
